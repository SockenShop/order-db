...
agent any
    stages {
        stage('Build') {
            ...
            sh 'docker build -t $DOCKER_ID/$DOCKER_IMAGE_ORDERS_DB:$DOCKER_TAG .'
                   }
        }
        stage('Run') {
            steps {
                sh 'docker network create $BUILD_TAG'
                sh 'docker run -d --name $DOCKER_IMAGE_ORDERS_DB --rm --network $BUILD_TAG $DOCKER_ID/$DOCKER_IMAGE_ORDERS_DB:$DOCKER_TAG'
                sh 'docker stop $DOCKER_IMAGE_ORDERS_DB'
        ...
        stage('Push') {
         ...
         steps {
            sh 'docker image tag $DOCKER_ID/$DOCKER_IMAGE_ORDERS_DB:$DOCKER_TAG $DOCKER_ID/$DOCKER_IMAGE_ORDERS_DB:latest'
            sh 'docker login -u $DOCKER_ID -p $DOCKER_PASS'
            sh 'docker push $DOCKER_ID/$DOCKER_IMAGE_ORDERS_DB:$DOCKER_TAG && docker push $DOCKER_ID/$DOCKER_IMAGE_ORDERS_DB:latest'
            }
        }
        stage('Deploy EKS') {
            environment {
